package com.gamesidebar.core.harness

/**
 * A dependency-free test harness.
 *
 * The core module is plain Kotlin/JVM, so its tests are written once against this tiny API and run
 * two ways:
 *  - on a device-less machine straight from `main` in `com.gamesidebar.core.tests.CoreTestSuitesKt`
 *    (see `tools/run_core_tests.sh`);
...
object Assert {
    ...
    fun close(expected: Double, actual: Double, tolerance: Double = 1e-9, message: String = "") {
    ...
    fun inRange(value: Int, min: Int, max: Int, message: String = "") {
